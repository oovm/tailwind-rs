...
// Hash of the grammar file: 394A0B361430B31673D901F2B13A7AC8214D1620914338E2367B520D73F4EB7E
...
impl peginator_generated::PegParser for TwFraction {
    fn parse_advanced<T: peginator_generated::ParseTracer>(
        s: &str,
        settings: &peginator_generated::ParseSettings,
    ) -> Result<Self, peginator_generated::ParseError> {
        Ok(peginator_generated::parse_TwFraction(
            peginator_generated::ParseState::new(s, settings),
            T::new(),
            &mut Default::default(),
        )?
...
mod peginator_generated {
    ...
    mod TwFraction_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: lhs, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_IntegerNode(state, tracer, cache))?;
            let ParseOk { state, .. } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_character_literal(state, '/'))
                .discard_result()?;
            let ParseOk { result: rhs, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_IntegerNode(state, tracer, cache))?;
            let ParseOk { state, .. } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_end_of_input(state))?;
            Ok(ParseOk { result: Parsed { lhs, rhs }, state })
        }
        pub struct Parsed {
            pub lhs: IntegerNode,
            pub rhs: IntegerNode,
        ...
        pub fn rule_parser<'a>(
            ...
            tracer: impl ParseTracer,
            cache: &mut ParseCache<'a>,
        ) -> ParseResult<'a, super::TwFraction> {
            let result =
                parse(state, tracer, cache)?.map(|r| super::TwFraction { lhs: r.lhs, rhs: r.rhs });
            Ok(result)
        }
    }
    #[inline]
    pub(super) fn parse_TwFraction<'a>(
        state: ParseState<'a>,
        tracer: impl ParseTracer,
        cache: &mut ParseCache<'a>,
    ) -> ParseResult<'a, TwFraction> {
        tracer.run_traced("TwFraction", state, |state, tracer| {
            TwFraction_impl::rule_parser(state, tracer, cache)
        })
    }
    ...
    mod ElementNode_impl {
        ...
        pub fn parse<'a>(
            ...
            ChoiceHelper::new(state)
                ...
                .choice(|state| {
                    parse_Whitespace(state, tracer, cache)
                        .and_then(|ParseOk { state, .. }| parse_SelfRef(state, tracer, cache))
                        .map_inner(Some)
                        .map_inner(|r| Parsed { negative: None, identifiers: Vec::new(), zelf: r })
                })
                .end()
        }
        pub struct Parsed {
        ...
        pub fn rule_parser<'a>(
            ...
            let result = parse(state, tracer, cache)?.map(|r| super::ElementNode {
                negative: r.negative,
                identifiers: r.identifiers,
                zelf: r.zelf,
            });
    ...
    mod SelfRef_impl {
        ...
        pub fn rule_parser<'a>(
            ...
            tracer: impl ParseTracer,
            cache: &mut ParseCache<'a>,
        ) -> ParseResult<'a, super::SelfRef> {
            let result = parse(state, tracer, cache)?.map(|r| super::SelfRef {});
            Ok(result)
        }
    }
    #[inline]
    pub(super) fn parse_SelfRef<'a>(
        state: ParseState<'a>,
        tracer: impl ParseTracer,
        cache: &mut ParseCache<'a>,
    ) -> ParseResult<'a, SelfRef> {
        tracer.run_traced("SelfRef", state, |state, tracer| {
            SelfRef_impl::rule_parser(state, tracer, cache)
        })
    }
    mod Not_impl {
        use super::*;
    ...
    pub(super) fn parse_IntegerNode<'a>(
        ...
        tracer.run_traced("IntegerNode", state, |state, tracer| {
            IntegerNode_impl::rule_parser(state, tracer, cache)
        })
    }
    #[inline]
