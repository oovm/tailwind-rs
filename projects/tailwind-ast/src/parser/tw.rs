...
// Hash of the grammar file: 924462233A089AB056574B37B0FD3A2BF80E7A8BF40BD65F9B458BC94D37469F
...
pub struct VariantItemNode {
    pub identifiers: IdentifierNode,
    pub pseudo: Pseudo,
}
#[derive(Debug, Clone)]
...
mod peginator_generated {
    ...
    mod VariantItemNode_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: identifiers, state } = parse_Whitespace(state, tracer, cache)
            ...
            Ok(ParseOk { result: Parsed { identifiers, pseudo }, state })
        }
        pub struct Parsed {
            pub identifiers: IdentifierNode,
            pub pseudo: Pseudo,
        ...
        pub fn rule_parser<'a>(
            ...
            let result = parse(state, tracer, cache)?
                .map(|r| super::VariantItemNode { identifiers: r.identifiers, pseudo: r.pseudo });
            Ok(result)
        }
    }
    #[inline]
    ...
    mod ElementNode_impl {
        ...
        pub fn rule_parser<'a>(
            ...
            let result =
                parse(state, tracer, cache)?.map(|r| super::ElementNode { identifiers: r });
            Ok(result)
        }
    }
    #[inline]
